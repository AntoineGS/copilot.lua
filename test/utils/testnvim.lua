...
if os.getenv("OSV_PORT") then
  ...
  table.insert(
    M.nvim_argv,
    string.format("lua require('osv').launch({ port = %s, blocking = true })", os.getenv("OSV_PORT"))
  )
end
...
function M.connect(file_or_address)
  ...
  local stream = (addr and port) and SocketStream.connect(addr, port) or SocketStream.open(file_or_address)
...
function M.set_shell_powershell(fake)
  ...
  local cmd = "Remove-Item -Force "
    .. table.concat(
      is_os("win") and { "alias:cat", "alias:echo", "alias:sleep", "alias:sort", "alias:tee" } or { "alias:echo" },
      ","
    )
    .. ";"
  M.exec([[
...
function M.assert_visible(bufnr, visible)
  ...
  if visible then
    assert(-1 ~= M.fn.bufwinnr(bufnr), "expected buffer to be visible in current tabpage: " .. tostring(bufnr))
  else
    assert(-1 == M.fn.bufwinnr(bufnr), "expected buffer NOT visible in current tabpage: " .. tostring(bufnr))
  end
end
